...
//        imageMagick = require('imagemagick'),
        ...
        serve = function (req, res) {
            ...
            var handleResult = function (result, redirect) {
                    console.log('Start handling result: ' + JSON.stringify(result));
                    console.log('by redirect: ' + redirect);
                    if (redirect) {
                        res.writeHead(302, {
    ...
    UploadHandler.prototype.post = function () {
        var handler = this,
            ...
            redirect,
            finish = function () {
                console.log('Form: finish starts.');
                counter -= 1;
                ...
                console.log('Form: finish ends.');
            };
        form.uploadDir = options.tmpDir;
        form.on('fileBegin', function (name, file) {
            console.log('Form: fileBegin starts.');
            tmpFiles.push(file.path);
            var fileInfo = new FileInfo(file, handler.req, true);
            fileInfo.safeName();
            map[path.basename(file.path)] = fileInfo;
            files.push(fileInfo);
            console.log('Form: fileBegin ends.');
        }).on('field', function (name, value) {
            console.log('Form: field starts.');
            if (name === 'redirect') {
                redirect = value;
            }
            console.log('Form: field ends.');
        }).on('file', function (name, file) {
            console.log('Form: file \"' + file.path + '\" starts.');
...
*/
            console.log('Form: file \"' + file.path + '\" ends.');
        }).on('aborted', function () {
            ...
            console.log('Form: progress starts.');
            if (bytesReceived > options.maxPostSize) {
                handler.req.connection.destroy();
            }
            console.log('Form: progress ends.');
